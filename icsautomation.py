        ...
        with st.spinner("正在分析文件结构并处理数据..."):
            try:
                with tempfile.TemporaryDirectory() as tmp_dir:
                    ...
                    # --- 步骤 1: 处理 containerinformation (生成中间文件 P) ---
                    ...
                    for bill_no, group in grouped:
                        if bill_no == "nan" or not bill_no: continue
                    ...
                    # --- 步骤 2: 处理 realdoc 置换 (修正路径匹配) ---
                    with zipfile.ZipFile(realdoc_zip, 'r') as z:
                        z.extractall(r_dir)

                    # 建立 realdoc 文件的全路径索引 (解决文件夹层级问题)
                    realdoc_map = {}
                    ...
                    st.success(f"处理完成！生成 {len(p_files)} 个单号，成功置换 {match_count} 个文件。")
                    st.download_button("📥 下载结果", zip_buffer.getvalue(), "final_result.zip")
